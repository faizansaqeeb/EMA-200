...
# ================= EMA 9 TOUCH + EXPANSION =================
def ema9_touch_and_expand_long(df):
    ema9 = ema(df["close"], 9)
    ...
    touch_idx = None
    for i in range(1, 4):
        if abs(ema9.iloc[-i] - ema200.iloc[-i]) <= 0.35 * atr_val:
            touch_idx = -i
            break
    if touch_idx is None:
        return False

    # Minimum expansion after touch (KEY RULE)
    if (ema9.iloc[-1] - ema9.iloc[touch_idx]) < 0.5 * atr_val:
        return False

    # Distance from EMA 200 must increase
    if abs(ema9.iloc[-1] - ema200.iloc[-1]) <= abs(ema9.iloc[touch_idx] - ema200.iloc[touch_idx]):
        return False
...
def ema9_touch_and_expand_short(df):
    ema9 = ema(df["close"], 9)
    ema200 = ema(df["close"], 200)
    atr_val = atr(df).iloc[-1]
    ...
    if (ema9.iloc[touch_idx] - ema9.iloc[-1]) < 0.5 * atr_val:
        return False
...
send_telegram("🚀 EMA 9 TOUCH + EXPANSION SCREENER STARTED")
    ...
    for symbol in symbols:
        for tf, interval in TIMEFRAMES.items():
            try:
                ...
                if ema9_touch_and_expand_long(df):
                    direction = "LONG"
                elif ema9_touch_and_expand_short(df):
                    direction = "SHORT"
                else:
                    continue
                ...
                msg = (
                    f"🔥 <b>{direction} EMA 9 TOUCH + EXPANSION</b>\n\n"
                    f"🪙 <b>Coin:</b> {symbol}\n"
                    f"⏱ <b>TF:</b> {tf}\n"
                    f"🎯 <b>Score:</b> {score}%\n"
